...
def _determine_burst_structure(struct, match, within_burst_interval=.25, minimum_burst_duration=1,
    ...
    burst_onset = np.array([_spike_times[0] for _spike_times in bursts])
    burst_offset = np.array([_spike_times[-1] for _spike_times in bursts])

    if combine_close_long_bursts:
        burst_onset, burst_offset = _combine_close_bursts(burst_onset=burst_onset,
...
def _calc_total_spikes_from_phase_arr(neuron_list, all_spikes, phase_arr, phase_bins, date, condition):
    ...
    for i, (neuron, data) in enumerate(zip(neuron_list, all_spikes)):
        ...
        df = pd.DataFrame(nearest_phase_space, columns=['bin'])
        df['bin'] %= phase_bins
        df['# of Spikes'] = 1
        df = df.groupby('bin', sort=False, as_index=False).agg({'# of Spikes': np.sum})
        df['neuron'] = neuron
        df['date'] = date
        ...
        df['date'] = date
        df['condition'] = condition
        df = df.groupby(['bin', 'neuron', 'date', 'condition'], sort=False, as_index=False).agg({'# of Spikes': np.sum})
        df['trial number'] = df['bin'] // phase_bins
        df['bin'] %= phase_bins
        df = df.reset_index(drop=True)
        exp_data.append(df)

    agg_df = pd.concat(agg_exp_data).sort_values(['neuron', 'bin'])
    agg_df = agg_df.reset_index(drop=True)
    df = pd.concat(exp_data).sort_values(['neuron', 'bin'])
    df = df.reset_index(drop=True)
